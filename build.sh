...
function showHelpAndExit {
        ...
        echo -e "${CLR_BLD_BLU}  -z, --image_zip       Generate fastboot flashable image zip from signed target_files${CLR_RST}"
        exit 1
}
...
getopt_cmd=$(getopt -o hcirvz:t:j:m:s:p:b --long "$long_opts" \
            -n $(basename $0) -- "$@") || \
