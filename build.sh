...
# Build rom package
    ...
    cp -f $OUT/target/product/$DEVICE/pa_$DEIVCE-ota-$FILE_NAME_TAG.zip $OUT/target/product/$DEVICE/pa-$PA_VERSION.zip
    ...
    if [ "$FLAG_IMG_ZIP" = 'y' ]; then
        ...
        cp -f $OUT/target/product/$DEVICE/pa_$DEIVCE-img-$FILE_NAME_TAG.zip $OUT/target/product/$DEVICE/pa-$PA_VERSION-image.zip
    fi
fi
echo -e ""
