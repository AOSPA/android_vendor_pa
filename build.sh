...
function showHelpAndExit {
        ...
        echo -e "${CLR_BLD_BLU}  -v, --variant         PA variant - Can be release, alpha or beta${CLR_RST}"
...
if [ ! -d "$CURDIR/vendor/pa" ]; then
        echo -e "${CLR_BLD_RED}error: insane root directory ($CURDIR)${CLR_RST}"
        exit 1
fi
...
if [ "$FLAG_SYNC" = 'y' ]; then
        ...
        echo -e ""
        repo sync -j"$JOBS" -c --no-clone-bundle
fi
...
echo -e ""
PA_VERSION=$(lunch "pa_$DEVICE-$BUILD_TYPE" | tee /dev/tty | grep 'PA_VERSION=*' | sed 's/.*=//')
echo -e ""
...
# Build signed rom package if specified
    ...
    if [ "$FLAG_BACKUP_UNSIGNED" = 'y' ]; then
        ${MAKE} bacon"$CMD"
        mv $OUT/pa-${PA_VERSION}.zip $PWD/pa-${PA_VERSION}-unsigned.zip
    else
