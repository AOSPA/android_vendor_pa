...
class Gerrit:
    def __init__(self, url='https://gerrit.aospa.co'):
...
def run_repopick(changes, path=None, extra_args=[]):
    ...
    global pick_counts
    top = gettop()
    args = ['{}/vendor/pa/build/tools/repopick.py'.format(top)]
...
def generate_picklist(args, ok_repos):
    ...
    # TODO: don't try get changes for projects that aren't on pa gerrit (or, i guess, the supplied gerrit)
    picklist = Picklist()
