...
prebuilt_build_tools_aospa := prebuilts/tools-pa
prebuilt_build_tools_aospa_bin := $(prebuilt_build_tools_aospa)/$(HOST_PREBUILT_TAG)/bin

# Clear this first to prevent accidental poisoning from env
KERNEL_MAKE_FLAGS :=

# Add back threads, ninja cuts this to $(nproc)/2
KERNEL_MAKE_FLAGS += -j$(shell $(prebuilt_build_tools_aospa_bin)/nproc --all)
...
ifeq ($(HOST_OS),darwin)
  KERNEL_MAKE_FLAGS += C_INCLUDE_PATH=$(PWD)/external/elfutils/libelf:/usr/local/opt/openssl/include
  KERNEL_MAKE_FLAGS += LIBRARY_PATH=/usr/local/opt/openssl/lib
else
  KERNEL_MAKE_FLAGS += C_INCLUDE_PATH=$(PWD)/prebuilts/openssl/$(HOST_OS)-x86/1.1.1/include
  KERNEL_MAKE_FLAGS += LIBRARY_PATH=$(PWD)/prebuilts/openssl/$(HOST_OS)-x86/1.1.1/lib/x86_64-linux-gnu
  KERNEL_MAKE_FLAGS += HOSTCFLAGS="-L $(PWD)/prebuilts/openssl/$(HOST_OS)-x86/1.1.1/lib/x86_64-linux-gnu"
  KERNEL_MAKE_FLAGS += HOSTLDFLAGS="-L $(PWD)/prebuilts/openssl/$(HOST_OS)-x86/1.1.1/lib/x86_64-linux-gnu"
endif
...
TOOLS_PATH_OVERRIDE := \
    PATH=$(PWD)/$(prebuilt_build_tools_aospa_bin):$$PATH \
    LD_LIBRARY_PATH=$(PWD)/$(prebuilt_build_tools_aospa)/$(HOST_PREBUILT_TAG)/lib:$$LD_LIBRARY_PATH \
    PERL5LIB=$(PWD)/$(prebuilt_build_tools_aospa)/common/perl-base
...
KERNEL_DTC_CMD := $(PWD)/prebuilts/build-tools/$(HOST_OS)-x86/bin/dtc
