...
REPOS_TO_MERGE = {"manifest": "platform/manifest"}
REPOS_RESULTS = {}
...
def print_proper_usage():
    ...
    print(
        "Usage: python3 vendor/pa/scripts/merge-aosp.py <REVISION> [-<PROJECT1> -<PROJECT2>...]"
    )
...
def read_custom_manifest(default_repos):
    ...
    with open("{0}/.repo/manifests/{1}".format(WORKING_DIR, MANIFEST_NAME)) as manifest:
        ...
        for custom in root:
            if custom.tag == "project":
                custom_name = custom.get("name")
                if custom_name and custom_name in default_repos:
                    if custom.get("path") not in BLACKLIST:
                        REPOS_TO_MERGE[custom.get("path")] = custom_name
